-- Drop e recriar função get_temporal_loss_reasons_report com colunas Total N e Total %
DROP FUNCTION IF EXISTS public.get_temporal_loss_reasons_report(text, text, uuid[], uuid[], uuid);
...
RETURNS TABLE(
  ...
  total_n bigint, total_percent numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- CORREÇÃO: Calcular total de motivos por mês (não clientes únicos)
  totais_por_mes AS (
    SELECT 
      TO_CHAR(clr.created_at, 'YYYY-MM') as mes_ano,
      COUNT(*)::bigint as total_motivos_mes  -- Total de registros de loss_reasons no mês
    FROM client_loss_reasons clr
    WHERE 
  ...
  -- Calcular dados agrupados por motivo
  dados_agrupados AS (
    SELECT 
      fd.motivo_perda,
      ...
      ROUND(COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[1] THEN fd.quantidade END), 0)::numeric / 
            NULLIF(MAX(CASE WHEN tpm.mes_ano = mes_headers[1] THEN tpm.total_motivos_mes END), 0) * 100, 1) as mes_1_percent,
      ...
      ROUND(COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[2] THEN fd.quantidade END), 0)::numeric / 
            NULLIF(MAX(CASE WHEN tpm.mes_ano = mes_headers[2] THEN tpm.total_motivos_mes END), 0) * 100, 1) as mes_2_percent,
      ...
      ROUND(COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[3] THEN fd.quantidade END), 0)::numeric / 
            NULLIF(MAX(CASE WHEN tpm.mes_ano = mes_headers[3] THEN tpm.total_motivos_mes END), 0) * 100, 1) as mes_3_percent,
      ...
      ROUND(COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[4] THEN fd.quantidade END), 0)::numeric / 
            NULLIF(MAX(CASE WHEN tpm.mes_ano = mes_headers[4] THEN tpm.total_motivos_mes END), 0) * 100, 1) as mes_4_percent,
      ...
      ROUND(COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[5] THEN fd.quantidade END), 0)::numeric / 
            NULLIF(MAX(CASE WHEN tpm.mes_ano = mes_headers[5] THEN tpm.total_motivos_mes END), 0) * 100, 1) as mes_5_percent,
      ...
      ROUND(COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[6] THEN fd.quantidade END), 0)::numeric / 
            NULLIF(MAX(CASE WHEN tpm.mes_ano = mes_headers[6] THEN tpm.total_motivos_mes END), 0) * 100, 1) as mes_6_percent,
      ...
      (COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[1] THEN fd.quantidade END), 0) +
       ...
       COALESCE(SUM(CASE WHEN fd.mes_ano = mes_headers[6] THEN fd.quantidade END), 0))::bigint as total_n
      
    FROM filtered_data fd
    CROSS JOIN totais_por_mes tpm
    GROUP BY fd.motivo_perda
  ),
  -- Calcular total geral para percentage do Total %
  total_geral AS (
    SELECT SUM(total_n) as total_geral_n
  ...
  SELECT 
    ...
    da.total_n,
    -- Total % (percentual da linha em relação ao total geral)
    ROUND(da.total_n::numeric / NULLIF(tg.total_geral_n, 0) * 100, 1) as total_percent
  FROM dados_agrupados da
  CROSS JOIN total_geral tg
  ORDER BY da.total_n DESC, da.motivo_perda;
END;
$function$