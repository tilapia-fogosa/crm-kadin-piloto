-- LOG: Correção da função get_horarios_aula_inaugural - usar coluna prioridade real
-- Remove referência à coluna 'tipo' inexistente e usa 'prioridade' (INTEGER) existente
...
BEGIN
  ...
  professores_disponiveis AS (
    -- CORREÇÃO: Usar coluna prioridade real (INTEGER) ao invés de tipo inexistente
    SELECT 
      p.id,
      p.nome,
