-- Recriar a função get_loss_reasons_report sem a coluna sem_status_anterior
DROP FUNCTION IF EXISTS public.get_loss_reasons_report(text, text, uuid[], uuid[], uuid);
...
BEGIN
  ...
  -- Query principal com pivot (excluindo registros sem previous_status)
  RETURN QUERY
  WITH filtered_data AS (
    ...
    WHERE 
      ...
      -- Excluir registros sem previous_status
      AND clr.previous_status IS NOT NULL
    ...
    GROUP BY lr.name, clr.previous_status
  )
  SELECT 
    fd.motivo_perda,
