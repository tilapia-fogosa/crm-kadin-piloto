-- LOG: Migration para adicionar verificação de turmas regulares aos slots de aula inaugural
-- Objetivo: Impedir que slots ocupados por turmas regulares apareçam como disponíveis
-- Data: 2025-11-03
-- Correção: Adiciona CTE turmas_bloqueadas e integra na verificação de conflitos
...
BEGIN
  ...
  turmas_bloqueadas AS (
    ...
    WHERE t.active = true
      ...
      AND t.dia_semana = v_dia_semana_texto
  ),
  ...
  agendamentos_bloqueados AS (
    ...
    -- Fonte 3: Turmas regulares do dia da semana (NOVO!)
    SELECT 
      tb.professor_id,
      tb.sala_id,
      tb.horario_inicio,
...
COMMENT ON FUNCTION get_horarios_aula_inaugural IS 'Retorna slots disponíveis para aula inaugural considerando: 1) Eventos pontuais de professores, 2) Eventos pontuais de salas, 3) Turmas regulares do dia da semana. Verifica conflitos de horário entre professor/sala ocupados e slots disponíveis.';