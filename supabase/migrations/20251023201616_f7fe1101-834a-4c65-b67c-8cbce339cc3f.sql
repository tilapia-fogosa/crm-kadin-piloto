-- Migration: Filtrar tipos de atividade não mapeados em get_user_productivity_stats
-- Objetivo: Excluir 'Matrícula' e outros tipos não mapeados para evitar erro 22023 (field name must not be null)
-- Problema: jsonb_object_agg() recebia NULL como chave quando tipo_atividade não estava no CASE
-- Solução: Adicionar WHERE clause na CTE aggregated_stats para filtrar apenas os 4 tipos suportados
  ...
  ELSE
    ...
    IF EXISTS (
      ...
      IF p_user_ids IS NOT NULL AND array_length(p_user_ids, 1) > 0 THEN
        ...
        RAISE NOTICE '🔍 [DIAGNOSTIC] Usando filtro de usuários específico: %', v_target_user_ids;
      ELSE
        -- Franqueado sem filtro de usuários: buscar todos da(s) unidade(s)
        SELECT ARRAY_AGG(DISTINCT id)
        INTO v_target_user_ids
        FROM profiles
        WHERE unit_id = ANY(p_unit_ids);
