-- LOG: Correção dos nomes de colunas - professores usa 'status', salas usa 'active'
-- Problema: professores.active não existe, deve ser professores.status
-- Data: 2025-11-03
...
BEGIN
  ...
  turmas_bloqueadas AS (
    ...
    WHERE t.active = true
      ...
      -- Cast explícito do ENUM para TEXT
  ...
  -- CTE 3: Professores disponíveis da unidade (CORREÇÃO: usar 'status' em vez de 'active')
...
COMMENT ON FUNCTION get_horarios_aula_inaugural IS 'Retorna slots disponíveis para aula inaugural considerando: 1) Eventos pontuais de professores, 2) Eventos pontuais de salas, 3) Turmas regulares do dia da semana. CORREÇÃO: professores usa coluna status, salas usa coluna active.';